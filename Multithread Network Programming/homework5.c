...
char * request_str_ok = "HTTP/1.0 200 OK\r\n"
        "Content-type: %s; charset=UTF-8\r\n\r\n";
char * request_str_folder = "HTTP/1.0 400 Bad Request\r\n"
"Content-type: text/html; charset=UTF-8\r\n\r\n";

char * request_str_file_not_found = "HTTP/1.0 404 Not Found\r\n"
"Content-type: text/html; charset=UTF-8\r\n\r\n";
...
void* serve_request(void * targ){

  int client_fd = ((struct thread_arg *)targ)->client_fd;
  char * base_dir = ((struct thread_arg *)targ)->base_dir;
  int read_fd;
  int bytes_read;
  int file_offset = 0;
  ...
  filename[0] = '.';
  filename[1] = '/';
  strncpy(&filename[2],base_dir, 4094);
  strncpy(&filename[2 + base_len], requested_file, 4094 - base_len);
  struct stat * st = malloc(sizeof(struct stat));
  int retval = stat(filename, st);
  if (retval) {
    // File doesn't exist
    send(client_fd,request_str_file_not_found,strlen(request_str_file_not_found),0);
  } else if (S_ISDIR(st->st_mode)) {
    // Folder is requested
    send(client_fd,request_str_folder,strlen(request_str_folder),0);
  } else {
    // File exists
    char * contentType;
    contentType = getContentType(getFileExtension(filename));
    // write(1, filename, 10);
    // write(1, " ", 1);
    // write(1, contentType, strlen(contentType));
    // write(1, "\n", 1);
    char res_str[4096];
    sprintf(res_str, request_str_ok, contentType);
...
int main(int argc, char** argv) {
    ...
    /* Read the directory to serve files from the second command line argument */
    char* dir = (char *)malloc(strlen(argv[2]) * sizeof(char));
    strcpy(dir, argv[2]);
    ...
    while(1) {
        ...
        arguments->base_dir = dir;
        int retval = pthread_create(thread, NULL, serve_request, (void*) arguments);
